...
// Cache Lite options
$cache_options = array(
'cacheDir' => '/tmp/cachelite/',  // Directory to place cache files
'lifeTime' => 3600,            // Update every hour since we display the pricing
'pearErrorMode' => CACHE_LITE_ERROR_DIE  // Causes cachelite to print error msg and die on error
);
...
// Check for cached page.  Either ASIN+PAGE# or "main"+PAGE#
// If we have a hit, display it and exit
$cache_id = (!is_null($asin)) ? $asin.$page : 'main'.$page;
if ($layout = $CL->get($cache_id)) {
    echo $layout;
    exit(0);
}
...
// Buffer the output
ob_start();
require_once("4-3layout.php");

// Fetch the buffered output
$buffer = ob_get_contents();
ob_end_clean();

// Save the buffered output to the cache
$CL->save($buffer);
